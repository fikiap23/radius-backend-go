...
func (uq *UserQuery) QueryOauthAccounts() *UserOAuthAccountQuery {
	query := (&UserOAuthAccountClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithOauthAccounts(opts ...func(*UserOAuthAccountQuery)) *UserQuery {
	query := (&UserOAuthAccountClient{config: uq.config}).Query()
...
func (uq *UserQuery) loadOauthAccounts(ctx context.Context, query *UserOAuthAccountQuery, nodes []*User, init func(*User), assign func(*User, *UserOAuthAccount)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(useroauthaccount.FieldUserID)
	}
	query.Where(predicate.UserOAuthAccount(func(s *sql.Selector) {
